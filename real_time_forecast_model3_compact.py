...
region_load_forecast = load_forecast[
    load_forecast["zone"].isin(["WEST_REGION", "SOUTH_REGION", "MIDATL_REGION"])
].copy()

ugi_mw = (
    zone_load_forecast.loc[
        zone_load_forecast["zone"] == "UGI",
        ["forecast_datetime_ending_ept", "forecast_load_mw"]
    ]
    .rename(columns={"forecast_load_mw": "ugi_forecast_load_mw"})
)

zone_load_forecast = zone_load_forecast.merge(
    ugi_mw, on="forecast_datetime_ending_ept", how="left"
)
...
max_dt_per_zone = hourly_zone_load.groupby("zone")["datetime_ending_ept"].max()
...
hourly_zone_load = pd.concat(
    [
        hourly_zone_load,
        append_load_new[["datetime_ending_ept", "zone", "MW", "nerc_region", "mkt_region"]],
    ],
    ignore_index=True,
).sort_values("datetime_ending_ept").reset_index(drop=True)

ovec_mw = (
    hourly_zone_load.loc[
        hourly_zone_load["zone"] == "OVEC",
...
zonal_wcl["temp_total_variation_6h"] = zonal_wcl["temp_diff_1h_abs"].rolling(6).sum()
zonal_wcl["temp_total_variation_12h"] = zonal_wcl["temp_diff_1h_abs"].rolling(12).sum()
zonal_wcl["temp_total_variation_24h"] = zonal_wcl["temp_diff_1h_abs"].rolling(24).sum()
zonal_wcl["temp_fcst_1h"] = zonal_wcl["temperature_2m"].shift(-1)

horizon = 6
zonal_wcl["temp_lead_mean_6h"] = sum(
    zonal_wcl["temperature_2m"].shift(-i) for i in range(1, horizon + 1)
) / horizon
...
for zone in zones:
    ...
    while current_date <= end_date:
        ...
        forecast_df = forecast_df.rename(
            columns={"forecast_datetime_ending_ept": "datetime_ending_ept"}
        )
...
final_results[
    ...
    f"data/prediction/zone_prediction_{start_str}_to_{end_str}_at_{run_str}_try.csv",
    index=False,
)